...
void backlight_init_ports(void) {
  ...
  // pwmEnableChannel(&PWMD2, 0, PWM_FRACTION_TO_WIDTH(&PWMD2, 0xFFFF,cie_lightness(0xFFFF)));
...
void backlight_set(uint8_t level) {
    ...
    if (level == 0) {
      ...
      // Turn backlight on
      if(!is_breathing()){
        pwmEnableChannel(&PWMD2, 0, PWM_FRACTION_TO_WIDTH(&PWMD2,0xFFFF,duty));
      }
    }
}
...
bool is_breathing(void) {
    return PWMD2.config == &pwmCFG_breathing;
}
...
void breathing_interrupt_enable(void){
    pwmStop(&PWMD2);
    pwmStart(&PWMD2, &pwmCFG_breathing);
    chSysLockFromISR();
